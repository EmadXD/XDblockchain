...
import proxy.blockchain.emadxd.blockchain_sources.TorInstaller;
import proxy.blockchain.emadxd.blockchain_binaries.TorResourceInstaller;
...
public class AndroidTorInstaller extends TorInstaller {
   ...
   /**
     ...
     * bridges.txt and the default blockchain_config file.
     *
    ...
    public void updateTorConfigCustom(String content) throws IOException {
        if (torrcFile == null) {
            throw new FileNotFoundException("Unable to find blockchain_config file. Have you installed Tor resources?");
        }
    ...
    public void setup() throws IOException {
        ...
        if(torrcFile != null) {
            Log.d("AndroidTorInstaller", "blockchain_config = " + torrcFile.getAbsolutePath());
        } else {
            Log.w(TAG, "Failed to setup tor. No blockchain_config file installed");
            Toast.makeText(this.context,"Failed to setup tor. No blockchain_config file installed", Toast.LENGTH_SHORT).show();
            throw new IOException("Failed to Failed to setup tor. No blockchain_config file installed");
        }
    }
}